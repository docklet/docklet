...
<script src="dl-wssh.js"></script>
<script>
	var client = new WSSHClient();

	client.connect({
		proxyhost: 'docklet.unias.org:5000',
		
		username: 'test',
		hostname: 'localhost',
		authentication_method: 'password', // can either be password or private_key
		password: 'test', // do not provide when using private_key
...
WSSHClient.prototype._generateEndpoint = function(options) {
    ...
    var endpoint = protocol + options.proxyhost + '/wssh/' + encodeURIComponent(options.hostname) + '/' + encodeURIComponent(options.username);
